        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="min-w-0">
            ...
            <div className="mt-6 flex flex-wrap gap-3 text-xs uppercase tracking-[0.2em] text-muted-foreground">
              ...
              <span className="rounded-full border border-white/10 bg-black/40 px-4 py-2">
                {prompt.copy_count} {dict.common.copiesSuffix}
              </span>
              <span className="rounded-full border border-white/10 bg-black/40 px-4 py-2">
                {publishedLabel}
              </span>
            ...
            <div className="mt-10">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {parameters.length > 0 ? (
                  ...
                  <div className="glass-panel rounded-xl px-4 py-6 text-sm text-muted-foreground">
                    {dict.prompt.parametersEmpty}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="min-w-0 space-y-6">
            <div className="glass-panel rounded-2xl p-5">
              ...
              <div className="mt-4 grid gap-3">
                {prompt.preview_images?.length ? (
                  <>
                    ...
                    {prompt.preview_images.length > 1 && (
                      <div className="grid gap-3 sm:grid-cols-2">
                        {prompt.preview_images.slice(1, 3).map((image, index) => (
                          <div
                  ...
                  <div className="flex h-52 items-center justify-center rounded-xl border border-white/10 bg-black/40 text-sm text-muted-foreground">
            ...
            <div className="glass-panel rounded-2xl p-5">
              <p className="text-xs uppercase tracking-[0.3em] text-primary/70">
                {dict.prompt.metadataLabel}
              </p>
              <div className="mt-4 space-y-3 text-sm text-muted-foreground">
                <div className="flex items-center justify-between">
                  <span>{dict.prompt.metadataAuthor}</span>
                  <span className="text-foreground">
                    {author?.name ?? dict.prompt.curatedBy}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span>{dict.prompt.metadataSlug}</span>
                  <span className="text-foreground">{prompt.slug}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>{dict.prompt.metadataStatus}</span>
                  <span className="text-foreground">{prompt.status}</span>
                </div>
              </div>
            </div>
          </div>
